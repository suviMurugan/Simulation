...
% ~MinDeg is the Minimum Degree with which a node is added.
...
total=sum([MinDeg:100].^-2.2);
for i=1:2*Iter                                                             %Calculating values for approximation
 %-------------------------------------------------------------------------
   change = 0;
   range_prob = ([MinDeg : 100].^1-2.2)/total;
   THETA=sum(w(1)*range_prob)./sum(range_prob);
   for DegDistro = MinDeg : 100
    k1 = h*Infector(t(i)    , w(i,:)      ,Rate ,DegDistro,VecRate,THETA);
    k2 = h*Infector(t(i)+h/2, w(i,:)+k1/2 ,Rate ,DegDistro,VecRate,THETA);
    k3 = h*Infector(t(i)+h/2, w(i,:)+k2/2 ,Rate ,DegDistro,VecRate,THETA);
    k4 = h*Infector(t(i)+h  , w(i,:)+k3   ,Rate ,DegDistro,VecRate,THETA);
    change = change + ((k1 + 2*k2 + 2*k3 + k4) / 6)*((DegDistro.^-2.2) /total);                    %Weighted average for the next value
   end
 %-------------------------------------------------------------------------
    w(i+1,:) = w(i,:) + change;
    t(i+1) = t(i) + h;
...
%%%%%%%%%%%%%%%%%%
function v = Infector(t,I,r,deg,vrate,THETA)                                       %Functions to be plotted
rho=sum(I(1:end-1));
  v(1) = - r(1)*I(1) + r(end)*deg*(1-rho)*(THETA) + vrate(1)*(1-rho)*I(end);   %Seperate for state 1
    for j=2:length(I)-1
